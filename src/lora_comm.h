...
#define LORA_RECEIVER_SERIAL_BPS 115200
...
void setup_transmitter_config() {
  ...
  c = E32_transmitter.getConfiguration();
  Configuration configuration = *(Configuration*) c.data;
  Serial.println(c.status.getResponseDescription());
  Serial.println(c.status.code);

  print_E32_parameters(configuration);
  configuration.CHAN = TRANSMITTER_CHANNEL;
  configuration.ADDH = TRANSMITTER_ADDH;
  configuration.ADDL = TRANSMITTER_ADDL;
  ...
  ResponseStatus rs = E32_transmitter.setConfiguration(configuration, WRITE_CFG_PWR_DWN_SAVE);
